...
from neural_ir.src.index import FaissFlatSearcher
    ...
    with open(ranking_save_file, 'w') as f:
        for qid, q_doc_scores, q_doc_indices in zip(q_lookup, corpus_scores, corpus_indices):
            ...
            for s, idx in score_list:
                rank += 1
                f.write(f'{qid} Q0 {idx} {rank} {s} dpr\n')
...
def main():
    ...
    parser.add_argument('--exclued_cf_doc', action='store_true')

    args = parser.parse_args()

    index_files = glob.glob(args.passage_reps)
    if args.exclued_cf_doc:
        index_files = [file for file in index_files if 'fake' not in file]

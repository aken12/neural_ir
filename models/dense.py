...
class DenseModel(EncoderModel):
    ...
    def encode_query(self, queries):
        query_hidden_states = self.encoder(**queries, return_dict=True)
        query_hidden_states = query_hidden_states.last_hidden_state
        return self._pooling(query_hidden_states, queries['attention_mask'])
    
    def encode_passage(self, passages):
        return self.encode_query(passages)
